...
if __name__ == "__main__":
    ...
    for k, T in enumerate(obs_win_list):
        ...
        for i, item in enumerate(fig_list):
            ...
            for ii, nKL_name in enumerate(nKL_name_list):
                ...
                fname = '_output/dae_sjdf_test_{:s}_e{:04d}.npy'.format(
                                                         nKL_name, epoch)
    ...
    loss_all = np.zeros((2, nensemble, nepochs))

    for k, T in enumerate(obs_win_list):
        for n in range(nensemble):
            fname = '_output/dae_sjdf_train_loss_{:02d}_{:02d}.npy'\
                    .format(T, n)
            loss_all[k, n, :] = np.load(fname)
    ...
    label_list = []
    for k, T in enumerate(obs_win_list):
        line0, = ax.semilogy(loss_mean[k, :], 
                            color=colors[k])

        line1 = ax.fill_between(np.arange(nepochs), 
                loss_mean[k, :] - loss_2std[k, :],
                loss_mean[k, :] + loss_2std[k, :],
                color=colors[k],
                alpha=0.3,
                #edgecolor='k',
        ...
        label_list.append('{:s} \t\t({:02d}mins)'\
                          .format('$\mathcal{L}$ mean ',t_pred))
        label_list.append('{:s} \t({:02d}mins)'\
                          .format('$\mathcal{L}$ mean $\pm$ 2 std',t_pred))
    ...
    fname = 'dae_training_loss.{:s}'.format(fmt)
    save_fname = os.path.join('_plots', fname)
    fig.tight_layout()
    ...
    for runno in test_runno:
        ...
        fig.tight_layout()
        fname = 'dae_timeseries_test_r{:04d}.{:s}'.format(runno, fmt)
    ...
    for nKL_name in nKL_name_list:
        ...
        fname = 'dae_sjdf_obs_{:s}.npy'.format(nKL_name)
        ...
        for k, T in enumerate(obs_win_list):
            ...
            fname = 'dae_sjdf_test_{:s}_e{:04d}.npy'.format(nKL_name, epoch)
            load_fname = os.path.join('_output', fname)
        ...
        fig.tight_layout()
        fname = 'dae_timeseries_{:s}.png'.format(nKL_name)
        fname = os.path.join('_plots', fname)
        fig.savefig(fname, dpi=dpi)
    ...
    fname = 'dae_timeseries_test_r{:04d}_ens.png'.format(runno)
    fname = os.path.join('_plots', fname)
    fig.savefig(fname, dpi=dpi)
